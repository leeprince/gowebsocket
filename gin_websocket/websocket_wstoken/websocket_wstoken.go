...
func (manager *ClientManager) Start() {
	for {
		log.Println("<---管道通信:websocket_serverpush.go--->")
		select {
		...
		case message := <-Manager.Broadcast:
			...
			for id, conn := range Manager.Clients {
				if id != creatId(MessageStruct.WsToken) {
					continue
				}
				select {
...
func creatId(token string) string {
	return token
}
